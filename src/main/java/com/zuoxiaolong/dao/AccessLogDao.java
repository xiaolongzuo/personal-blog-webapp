    ...
    public static boolean save(final String visitorIp, final String url, final String params) {
        return execute(new TransactionalOperation<Boolean>() {
            ...
            public Boolean doInConnection(Connection connection) {
                try {
                    PreparedStatement statement = connection.prepareStatement("insert into access_log (visitor_ip,url,access_date,city,params) values (?,?,?,?,?)");
                    statement.setString(1, visitorIp);
                    statement.setString(2, url);
                    statement.setTimestamp(3, new Timestamp(System.currentTimeMillis()));
                    statement.setString(4, HttpApiHelper.getCity(visitorIp));
